...
def login_success(request):
    ...
    if request.method == "POST":
        ...
        if ((username != '') and (password == '')):  # ID: 입력, PW: 미입력
            ...
            return render(request, 'login.html')
            # error_titles = "Password 입력"
            # errors = "Password를 입력해 주세요"
            # return render(request, 'login.html', {'error_titles':error_titles,'errors':errors})
        if user is not None:
            ...
            logined_id = user_id # 지금은 user_id지만 추후 user.profile.player_serial로 변경
            ...
            for i in range(len(list_blob)):
                if(logined_id == list_blob[i]):
                    ...
                    # 타임테이블(마지막 하나 남기고), READALL (2개 빼고), MEDIA/image , MEDIA/video 삭제
...
def get_root_json_from_GCP(bucket_name=using_bucket):  # download the json
