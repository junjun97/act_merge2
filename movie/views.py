...
def movie(request):
    ...
    list_dict = {
        ...
        'list_videoindex' : move_movie_index,  #비디오 위아래 이동시 인덱스
        }
    ...
    if request.method == 'POST':
        if request.is_ajax():
            ...
            if (play_list_index >= len(play_list)):
                ...
                global move_movie_index
                move_movie_index = video_index + 1
            ...
            if (play_list_index >= len(play_list)):
                ...
                if (len(thisvideo_schedule) == 14):
                    if (len(prevideo_schedule) == 14):
                        thisvideo_schedule_makedate = thisvideo_schedule[:14]
                        prevideo_schedule_makedate = nextvideo_schedule[:14]
...
def directory_list():   # 디렉토리가 '/'로 끝나는 특징을 사용해 디렉토리 이름만 추출
    play_list_name = play_list_in_bucket(using_bucket)  # 중복 확인을 위해 PLAY_LIST 하위 이름들의 배열
    list_name = []
    ...
    if request.method == 'POST':
        if request.is_ajax():
            ...
            if (delete_index >= len(play_list)):
                ...
                delete_video_list = video_list_in_bucket_include_code(delete_name)  # 하위 객체들을 먼저 지우기 위함
                print('delete_video_list --> {}'.format(delete_video_list))
                delete_schedule_list = schedule_list_in_bucket(delete_name, -2)
