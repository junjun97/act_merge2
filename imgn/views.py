...
def downmove_picture(request):
    ...
    picture_index = this_picture_index + 1
...
def upmove_picture(request):
    ...
    this_picture_index = int(request.POST['picture_index'])
    picture_index = this_picture_index - 1 #재로딩될때 인덱스를 위한 지역변수임
...
#-------------------- for text preview -------------------
def make_text_preview(text_data): # 텍스트 미리보기
